    ...
    <div class="col-md-2 col-sm-3 col-xs-12  content content-full text-center flex-center">
        <div>
            ...
            <div class="h1 font-w700 ">
                @if($product->type == 2)
                    @if($product->kindsHotel()->where('quantity','>',0)->first())
                        ${{number_format($product->kindsHotel()->where('quantity','>',0)->first()->pivot->price * ( 1 - ($product->discount/100)))}}*
                    @else
                        <small style="font-size: 18px;">No vacancy</small>
                    @endif
                    @else
